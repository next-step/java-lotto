...
public class LottoResultViewTest {
	...
	public void printWinningProbabilityTest() {
		LottoResultView.printWinningProbability(0.12345678f);
		LottoResultView.printWinningProbability(0.23456f);
		LottoResultView.printWinningProbability(0.01111111f);
		LottoResultView.printWinningProbability(0.523456f);
	}

	@Test
	@DisplayName("로또 2등은 메시지가 다르다.")
	public void getPrintLottoStatisticsMessageTest() {
		assertThat(LottoResultView.getPrintLottoStatisticsMessage(LottoReward.FIRST)).isEqualTo("%d개 일치 (%d원)- %d개");
		assertThat(LottoResultView.getPrintLottoStatisticsMessage(LottoReward.SECOND)).isEqualTo("%d개 일치, 보너스 볼 일치(%d원)- %d개");
		assertThat(LottoResultView.getPrintLottoStatisticsMessage(LottoReward.THIRD)).isEqualTo("%d개 일치 (%d원)- %d개");
	}
}
