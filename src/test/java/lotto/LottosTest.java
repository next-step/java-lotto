...
public class LottosTest {
    ...
    void winningStatisticsTest(int number1, int number2, int number3, int number4, int number5, int number6, int bonusNumber, String key, int expectedMatchCount) {
        ...
        LottoWinningStatistics lottoRanks = lottos.getWinningStatistics(new WinningLottoNumber(new LottoNumbers(winningLottoNumber), new LottoNumber(bonusNumber)));
        LottoRank expectedRank = LottoRank.valueOf(key);

        assertThat(lottoRanks.getLottoRanks().get(expectedRank)).isEqualTo(expectedMatchCount);
    }
