...
class LottoPaperTest {
    ...
    void 테스트_getWinningPrice_당첨_금액_반환한다(LottoPaper input, WinningPrice expected) {
        ...
        // then
        assertThat(input.getWinningPrice(testLottoNumbers())).isEqualTo(expected);
    }
    ...
    private static Stream<Arguments> provideTestArguments() { // argument source method
        return Stream.of(
                ...
                Arguments.of(new LottoPaper(testLottoNumbers(1,2,3,4,7,8)), WinningPrice.THIRD),
                Arguments.of(new LottoPaper(testLottoNumbers(1,2,3,7,8,9)), WinningPrice.FORTH),
