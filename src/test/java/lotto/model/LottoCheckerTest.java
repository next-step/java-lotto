...
class LottoCheckerTest {
    LottoChecker checker = new LottoChecker(new WinningNumbers(new Lotto(new LottoNumbers(Arrays.asList(1, 2, 3, 4, 5, 6))), 7));

    @Test
    @DisplayName("기본 생성 테스트")
    public void create() {
        assertThat(checker).hasFieldOrProperty("winningNumbers");
    }
    ...
    public static Stream<Arguments> provideCheck() {
        return Stream.of(
            ...
            Arguments.arguments(Arrays.asList(1, 2, 3, 4, 5, 7), LottoPlace.SECOND),
            Arguments.arguments(Arrays.asList(1, 2, 3, 4, 5, 9), LottoPlace.THIRD),
            Arguments.arguments(Arrays.asList(1, 2, 3, 4, 8, 9), LottoPlace.FOURTH),
            Arguments.arguments(Arrays.asList(1, 2, 3, 7, 8, 9), LottoPlace.FIFTH),
