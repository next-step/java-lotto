...
class WinningInformationTest {
    ...
    void findByMatchedInformation() {
        assertThat(WinningInformation.findByMatchedInformation(5, true)).isEqualTo(SECOND);
    }

    @Test
    @DisplayName("파라미터에 해당하는 WinningInformation 이 없다면 NO_PRIZE 를 반환한다.")
    void findByMatchedInformation_noPrize() {
        assertThat(WinningInformation.findByMatchedInformation(4, true)).isEqualTo(NO_PRIZE);
    }
}