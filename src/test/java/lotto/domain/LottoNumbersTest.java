...
class LottoNumbersTest {
    ...
    void generate_로또번호모음을_생성하여_반환한다() {
        ...
        List<LottoNumber> lottoNumbersValue = lottoNumbers.getLottoNumbers();

        assertThat(new HashSet<>(lottoNumbersValue)).hasSize(LOTTO_NUMBER_COUNT);
        lottoNumbersValue.forEach(lottoNumber -> assertThat(lottoNumber).isInstanceOf(LottoNumber.class));
    }

    @ParameterizedTest
    @CsvSource(value = {"2_500,2", "3_000,3", "3_500,3"})
    void calculateCount_입력받은_금액만큼에_해당하는_로또_장수를_반환한다(int money, int lottoSheetCount) {
        assertThat(LottoNumbers.calculateNumbersCount(money)).isEqualTo(lottoSheetCount);
    }
