...
class LottoMachineTest {
    ...
    public void LottoMachine_Should_Calculate_Execute_Count() {
        //given
        LottoMachine lottoMachine = new LottoMachine(new PurchaseRecord(10000));
    ...
    public void LottoMachine_Should_Compare_Lotto_With_WinningNumbers_To_Check_Equals_All() {
        //given
        LottoMachine lottoMachine = new LottoMachine(new PurchaseRecord(1000));
    ...
    public void LottoMachine_Should_Compare_Lotto_With_WinningNumbers_To_Check_Equals_Five_And_Bonus_Number() {
        ...
        //then
        assertThat(grade).isEqualTo(Grade.BONUS_GRADE);
    }
    ...
    public void LottoMachine_Should_Return_Winning_Statistics_Using_Auto_Lottos_and_Winning_Number()
        ...
        //given
        Lotto firstGradeLotto = Lotto.fromValues(Arrays.asList(1, 2, 3, 4, 5, 6));     //  1등 당첨 로또
        Lotto bonusGradeLotto = Lotto.fromValues(Arrays.asList(1, 2, 3, 4, 5, 45));     //  보너스 당첨 로또
        Lotto secondGradeLotto = Lotto.fromValues(Arrays.asList(1, 2, 3, 4, 5, 10));   //  2등 당첨 로또
        Lotto thirdGradeLotto = Lotto.fromValues(Arrays.asList(1, 2, 3, 4, 20, 10));   //  3등 당첨 로또
        Lotto fourGradeLotto = Lotto.fromValues(Arrays.asList(1, 2, 3, 30, 20, 10));   //  4등 당첨 로또
        List<Lotto> autoLottos = new ArrayList<>(
            Arrays.asList(firstGradeLotto, secondGradeLotto, thirdGradeLotto, fourGradeLotto, bonusGradeLotto));

        WinningNumbers winningNumbers = WinningNumbers.fromValues(Arrays.asList(1, 2, 3, 4, 5, 6), LottoNumber.FORTY_FIVE);

        LottoMachine lottoMachine = new LottoMachine(new PurchaseRecord(20000));
