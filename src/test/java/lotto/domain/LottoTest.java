...
class LottoTest {
    ...
    @Test
    void 로또수동발급확인() {
        Lotto lotto = Lotto.manualGenerate(new LottoNumbers(InputView.makeNumbers("33, 36, 31, 34, 35, 32")));
        LottoNumbers expectedLottoNumbers = new LottoNumbers(InputView.makeNumbers("31, 32, 33, 34, 35, 36"));
        assertAll(
                () -> assertThat(lotto.numbers().getValues().size()).isEqualTo(6),
