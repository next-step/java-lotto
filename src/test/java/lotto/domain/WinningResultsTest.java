...
public class WinningResultsTest {
    ...
    void check_winning_results_test() {
        ...
        WinningResults winningResults = new WinningResults(lottoPaper, "1, 2, 3, 4, 5, 6", 7);
    ...
    void check_winning_results_second_test() {
        ...
        Lotto lotto = lottoGenerator.generateLotto();
        LottoPaper lottoPaper = new LottoPaper(Arrays.asList(lotto));
        WinningResults winningResults = new WinningResults(lottoPaper, "1, 2, 3, 4, 5, 7", 6);

        assertThat(winningResults.winningReport().awardResult().get(Award.SECOND)).isEqualTo(1);
        assertThat(winningResults.winningReport().rateOfReturn()).isNotNull();
    }
}
