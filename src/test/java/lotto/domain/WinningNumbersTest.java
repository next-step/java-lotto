...
public class WinningNumbersTest {
        ...
        void 중복된_당첨번호를_입력했을_경우_에러() {
                assertThatThrownBy(() -> new WinningNumbers(new int[]{1, 2, 3, 4, 6, 6})).isInstanceOf(
                    IllegalArgumentException.class).hasMessage("6개의 당첨번호를 중복되지않게 입력해주세요.");
        }
        ...
        void 기존_입력한_당첨번호와_중복된_보너스_번호를_입력했을_경우_에러() {
                ...
                assertThatThrownBy(() -> winningNumbers.addBonusNumberInToTheWinningNumbers(6)).isInstanceOf(
                    IllegalArgumentException.class).hasMessage("기존 입력한 6개의 당첨번호와 중복되지않게 입력해주세요.");
        }
}
