...
class WinningNumbersTest {
    ...
    public void Winning_Numbers_Has_Winning_Number() {
        WinningNumbers winningNumbers = WinningNumbers.fromValues(List.of(10, 20, 30, 31, 32, 33), LottoNumber.FORTY_FIVE);
    ...
    @Test
    @DisplayName("전달 받은 로또번호가 5개가 일치하고 보너스 번호가 일치할 경우 보너스 등급을 반환한다.")
    public void Winning_Numbers_Should_Return_5th_If_2_Received_Numbers_Match_And_Bonus() {
        WinningNumbers winningNumbers = WinningNumbers.fromValues(List.of(10, 20, 30, 31, 32, 33), LottoNumber.FORTY_FIVE);
        Lotto lotto = Lotto.fromValues(List.of(10, 20, 30, 31, 32, 45));
