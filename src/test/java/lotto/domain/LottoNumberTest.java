...
class LottoNumberTest {
    ...
    void matchedNumberCount() {
        LottoNumber number = LottoUtils.lottoNumber("1, 2, 3, 4, 5, 6");
    ...
    private NumberMatcher listNumberMatcher(String sourceNumbers) {
        List<Integer> inumbers = Arrays.asList(sourceNumbers.split(",")).stream()
                .map(String::trim)
                .map(Integer::valueOf)
                .collect(Collectors.toList());

        return new ListNumberMatcher(inumbers);
    }
