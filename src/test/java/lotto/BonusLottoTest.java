...
class BonusLottoTest {
	...
	@Test
	@DisplayName(value = "일반 로또 게임에선 THIRD를 반환하지만, 보너스 로또 게임에서는 SECOND를 반환한다")
	void bonusCount() {
		...
		assertThat(winner.ranking(purchased1)).isEqualTo(Ranking.THIRD);
		assertThat(winner.ranking(purchased2)).isEqualTo(Ranking.THIRD);
