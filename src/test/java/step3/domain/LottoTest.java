...
public class LottoTest {
  ...
  void 로또_번호_중복_제거 (String lottoNumbers) {
    ...
    step2.domain.Lotto lotto = generateLotto(lottoNumbers);
  ...
  void 로또_번호_오름차순_검사 (step2.domain.Lotto lotto, String expected) {
    assertEquals(expected, lotto.stream()
                                .map(String::valueOf)
                                .collect(Collectors.joining(",")));
  }
  ...
  void 로또_번호_비교_테스트 (step2.domain.Lotto lotto1, Lotto lotto2, int expected) {
  ...
  private static Stream<Arguments> provideTwoLottoAndSames () {
    return Stream.of(
      Arguments.of(
        step2.domain.LottoGenerator.generateLotto("1,2,3,4,5,6"),
        step2.domain.LottoGenerator.generateLotto("1,2,3,7,8,9"),
        3
      ),
      Arguments.of(
        step2.domain.LottoGenerator.generateLotto("1,2,3,4,5,6"),
