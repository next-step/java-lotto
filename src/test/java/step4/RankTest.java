...
public class RankTest {
    ...
    @CsvSource({
        ...
        "5, SECOND, 30000000, true",
        "6, FIRST, 2000000000, false"
    })
    @DisplayName("Rank Enum 매칭 테스트")
    void testRankMatching(int matchCount, Rank expectedRank, BigInteger expectedPrize, boolean isBonusMatched) {
        Rank rank = Rank.of(new MatchResult(matchCount, isBonusMatched));
        assertAll(
            () -> assertEquals(expectedRank, rank),
            () -> assertEquals(expectedPrize, rank.parsePrizeIntoBigInteger())
        );
    }
