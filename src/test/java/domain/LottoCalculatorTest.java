...
public class LottoCalculatorTest {
    ...
    void setUp(){
        ...
        lottoNumbers.add(Arrays.stream(new int[]{1, 2, 3, 4, 5, 6})
                            .boxed()
                            .collect(Collectors.toList()));
        lottoNumbers.add(Arrays.stream(new int[]{4, 5, 6, 7, 8, 9})
                            .boxed()
                            .collect(Collectors.toList()));
    ...
    void calculateTest() throws Exception {
        List<Integer> winningNumbers = Arrays.stream(new int[]{4,5,6,11,12,13})
                                        .boxed()
                                        .collect(Collectors.toList());
