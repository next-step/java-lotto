...
public class InputView {
    ...
    private static final String ERROR_NON_NUMBER = "숫자만 입력해주세요.";
    private static final String ERROR_NON_POSITIVE_NUMBER = "0을 초과하는 숫자를 입력해주세요.";
    private static final String ERROR_EMPTY_INPUT = "빈 값이 입력되었습니다. 다시 입력해주세요.";
    private static final String ERROR_SIX_NUMBERS_REQUIRED = "알맞은 갯수의 번호를 입력해주세요 : ";
    ...
    private boolean checkValidAmount(String input) {
        boolean stopReceivingInput = false;
        try {
            stopReceivingInput = checkNotNull(input) && checkIsNumber(input) & checkPositiveNumber(input);
    ...
    private boolean checkCountOfNumbers(String[] inputs) throws IllegalArgumentException {
        if (inputs.length != WinningNumbers.NUMBER_COUNT) {
            throw new IllegalArgumentException(ERROR_SIX_NUMBERS_REQUIRED+WinningNumbers.NUMBER_COUNT+"개");
        }
        return true;
    }
}
