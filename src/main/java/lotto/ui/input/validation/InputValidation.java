...
public class InputValidation {
	public boolean isBonusBallValidation(int[] fix, int bonus) {
		int index = 0;
		if (fix[index++] == bonus) {
			throw new IllegalArgumentException("보너스 볼이 당첨 번호에 포함되어 있습니다.");
		}

		return isBallValidation(bonus);
	}

	public boolean isBallValidation(int bonus) {
	...
	public boolean isWinningSixBallValidation(int[] fixs) {
