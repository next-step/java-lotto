...
public class ResultView {
    ...
    public static void printWinningCount(Map<Integer, Long> winningCount, int lottoPrice) {
        ...
        float sum = 0;
        for (int i = 3; i < 7; i++) {
            System.out.println(String.format("%d개 일치 (%d원)- %d개", i, winningMoney[i - 3], winningCount.get(i)));
            sum += winningMoney[i - 3] * winningCount.get(i);
        }

        System.out.println(String.format("총 수익률은 %f입니다.(기준이 1이기 때문에 결과적으로 손해라는 의미임)", sum / lottoPrice));
    }
}
