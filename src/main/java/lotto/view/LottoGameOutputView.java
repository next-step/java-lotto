...
public class LottoGameOutputView {
  ...
  private static void printGameResult(int matchCount, LottoDrawResults result) {
    if (matchCount == MATCH_COUNT_PRINT_BONUS) {
      printGameResultWithBonus(result);
      return;
    }
  ...
  private static void printGameResultWithBonus(LottoDrawResults result) {
    printGameResultMessage(MATCH_COUNT_PRINT_BONUS, result.filterBonusBall(false));
    printGameResultMessageWithBonus(MATCH_COUNT_PRINT_BONUS, result.filterBonusBall(true));
  }
