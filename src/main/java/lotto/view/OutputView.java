...
public class OutputView {
    ...
    private static final String BUY_LOTTO_TICKET_MESSAGE = "총 %d 장의 로또 티켓을 구매했습니다.";
    ...
    private static final String PRICE_AND_LOTTO_COUNT_MESSAGE = "(%d 원) - %d 개";

    public OutputView() {
    }

    public static void printPublishedLottoResult(LottoTicketCount lottoTicketCount, PublishedLottoTicket publishedLottoTicket) {
        printLottoTicketCount(lottoTicketCount.getLottoTicketCount());
        printLottoNumbers(publishedLottoTicket.getPublishedLottoTicket());
    }

    private static void printLottoTicketCount(int ticketCount) {
        System.out.println(String.format(BUY_LOTTO_TICKET_MESSAGE, ticketCount));
    }
    ...
    private static void printWinningLottoResult(Map<WinningPrize, Integer> lottoResult) {
        ...
        lottoResult.entrySet().forEach(result -> {
            ...
            System.out.println(String.format(PRICE_AND_LOTTO_COUNT_MESSAGE, result.getKey().getPrize(), result.getValue()));
        });
        System.out.println();
    }
