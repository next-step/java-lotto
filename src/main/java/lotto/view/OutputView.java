...
public class OutputView {
    ...
    private static final String PRINT_REQUEST_MANUAL_LOTTO_COUNT = "수동으로 구매할 로또 수를 입력해 주세요.";
    private static final String PRINT_REQUEST_MANUAL_LOTTO_NUMBERS = "수동으로 구매할 번호를 입력해 주세요.";
    private static final String PRINT_SYSTEM_LOTTO_COUNT_FORMAT = "%d개를 구매했습니다.%n";
    ...
    public static void printLottoCount(int count) {
        System.out.printf(PRINT_SYSTEM_LOTTO_COUNT_FORMAT, count);
    }
    ...
    public static void printLottoNumber(Lotto lotto) {
        for (int j = 0; j < lotto.getLotto().size(); j++) {
            System.out.printf(PRINT_LOTTO_NUMBER_FORMAT, lotto.getLotto().get(j).getLottoNumber());
        }
    }
}
