...
public class LottoTickets {
    ...
    public static LottoTickets create(int purchaseAmount) {
        return create(purchaseAmount, 0, Collections.emptyList());
    }

    public static LottoTickets create(int purchaseAmount, Integer manualLottoCount, List<String> manualLottoNumber) {
        validate(purchaseAmount);

        List<LottoTicket> tickets = new ArrayList<>();

        if (manualLottoCount > 0) {
            ManualLottoTickets manualLottoTickets= new ManualLottoTickets(manualLottoNumber);
            tickets.addAll(manualLottoTickets.fetchManualLottoTickets());
        }

        int numberOfTickets = Math.floorDiv(purchaseAmount, PRICE_OF_LOTTO_TICKET) - manualLottoCount ;
        List<LottoTicket> autoGeneratedTickets = generateLottoTickets(numberOfTickets);
