...
public enum ResultScoreEnum_ref {
    ...
    ResultScoreEnum_ref(int countOfMatch, boolean matchBonus, int winningMoney, String printResult) {
    ...
    public static ResultScoreEnum_ref valueOf(int countOfMatch, boolean matchBonus) {
         Optional<ResultScoreEnum_ref> result = Arrays.stream(values())
                .filter(resultScoreEnum -> resultScoreEnum.countOfMatch == countOfMatch && resultScoreEnum.matchBonus == matchBonus)
                .findFirst();

        return result.orElseGet(()-> ResultScoreEnum_ref.MISS);
    }
}
