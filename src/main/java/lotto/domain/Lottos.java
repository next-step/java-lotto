...
public class Lottos {
    ...
    public static int getLottoCount(int totalPurchaseAmount) {
        if (totalPurchaseAmount % LOTTO_PRICE != 0) {
            throw new IllegalArgumentException("로또 구입 금액은 1000의 배수여야 함");
        }

        return totalPurchaseAmount / LOTTO_PRICE;
    }
    ...
    public static Lottos createLottos(int lottoCount, LottoNumberStrategy strategy) {
        ...
        List<Lotto> lottoList = IntStream.range(0, lottoCount)
                .mapToObj(n -> new Lotto(strategy.createNumber()))
