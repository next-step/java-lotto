...
public class Calculator {
    ...
    public static void calculate(Lottos lottos, WinningNumbers winningNumbers, GameResult result) {
        result.makeWinningStatistics(getTotalAmount(lottos, winningNumbers, result), lottos.getAmount());
    }

    private static long getTotalAmount(Lottos lottos, WinningNumbers winningNumbers, GameResult result) {
        ...
        for (Lotto lotto : lottos.getLottos()) {
            ...
            totalAmount += Rank.valueOf(count, bonus, result);
        }
        return totalAmount;
    }
}
