...
public class ResultView {
    ...
    public static void showRewards(Map<Prize, Integer> summarize) {
        ...
        for (Entry<Prize, Integer> prize : summarize.entrySet()) {
            System.out.println(prize.getKey().getCorrect() + "개 일치 (" + prize.getKey().getReward() + "원)- " + prize.getValue());
        }
    }
    ...
    public static void showRoi(BigDecimal roi) {
        ...
        if (i > 0) {
            System.out.println("총 수익률은 " + roi + "입니다.(기준이 1이기 때문에 결과적으로 " + PROFIT + "라는 의미임)");
            return;
        }

        System.out.println("총 수익률은 " + roi + "입니다.(기준이 1이기 때문에 결과적으로" + LOSS + "라는 의미임)");
    }
}
