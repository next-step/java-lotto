...
public class ConsoleResultView implements ResultView {
    ...
    private static final String MESSAGE_FORMAT_OF_ALL_LOTTERIES_RESULT = "당첨 통계\n" +
            "---------\n" +
            "%s\n" +
            "총 수익률은 %.2f입니다.(기준이 1이기 때문에 결과적으로 손해라는 의미임)";
    private static final String MESSAGE_FORMAT_OF_ONE_LOTTERY_RESULT = "%d개 일치 (%ld원)- %d개";
    ...
    public void printPurchasedLotteries(List<Lottery> lotteries) {
        ...
        final String formattedLotteries = lotteries.stream()
                ...
                .collect(Collectors.joining(DELIMITER_LOTTERIES, PREFIX_LOTTERY, POSTFIX_LOTTERY));
    ...
    private String formatLottery(Lottery lottery) {
        return lottery.stream()
                .map(String::valueOf)
                .collect(Collectors.joining(DELIMITER_LOTTERY));
    }
    ...
    private String formatLotteryResult(RewardType rewardType, int numberOfLotteries) {
        ...
        final long reward = rewardType.getReward() * numberOfLotteries;
