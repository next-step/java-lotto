package study.step2.utils;

public class MessageUtil {

  private MessageUtil() {}

  public static final String VALIDATOR_MONEY_MESSAGE = "구입금액은 1000원이상으로 입력하세요.";
  public static final String NUMBER_AVAILABLE_PURCHASES_MESSAGE = "구매 갯수는 전체 구매 갯수 이내로 입력하세요.";
  public static final String INPUT_MONEY_MESSAGE = "구입금액을 입력해 주세요.";
  public static final String INPUT_BONUS_NUMBER_MESSAGE = "보너스 볼을 입력해 주세요.";
  public static final String LOTTO_COUNT_MESSAGE = "개를 구매했습니다.";
  public static final String MANUAL_LOTTO_COUNT_MESSAGE = "수동으로 구매할 로또 수를 입력해 주세요.";
  public static final String LAST_LOTTO_NUMBER_MESSAGE = "지난 주 당첨 번호를 입력해 주세요.";
  public static final String VALIDATOR_NUMBER_MESSAGE = "번호의 개수는 6개여야 합니다.";
  public static final String LOTTO_RESULT_MESSAGE = "당첨 통계";
  public static final String LOTTO_NUMBER_DUPLICATED = "중복되는 숫자가 포함되어 있는지 확인해주세요.";
  public static final String LOTTO_NUMBER_INVALID_RANGE = "로또 번호 범위를 확인해주세요.";
  public static final String BONUS_NUMBER_INVALID_MESSAGE = "보너스 번호는 지난 담청 번호 6개와 같을 수 없습니다.";
  public static final String MANUAL_LOTTO_NUMBER_INPUT_MESSAGE = "수동으로 구매할 번호를 입력해 주세요.";
}
