...
* 입력받은 수동 로또들을 생성한다.
* 로또 구매에서 수동 제외한 개수만큼 자동 로또 번호를 생성한다.
* 당첨 번호가 유효한 입력인지 확인한다.
* 자동로또번호와 당첨 번호를 비교하여 번호 일치 개수를 센다.
    * 일반번호들과 일치개수와, 보너스번호 일치여부를 확인한다.
* 총 수익률(수익 / 구매비용, 소수점 2자리 이후 절삭)을 구한다.
## 기능 목록 (UI)
* 로또 구입 금액을 입력받는다.
* 로또 수동 구입 개수를 입력받는다.
* 로또 수동 번호들을 입력받는다.
* 로또 당첨 번호(일반번호들, 보너스번호)를 입력 받는다.
* 총 수익률(수익 / 구매비용, 소수점 2자리 이후 절삭)을 구한다.
...
3. 유효한 구입 금액인지 확인해서
    1. 유효하지 않으면 종료한다.
4. 로또 수동으로 구매할 개수 입력받는다.   
    1. 유효한 수동 개수인지 확인한다. (구입금액에서 가능한지)
5. 수동 로또번호를 입력받는다.
6. 자동 로또 번호를 생성(구매)한다.
7. 지난 주 당첨 번호를 입력 받는다.
    1. 일반번호들 입력받는다.
    2. 보너스번호 입력받는다.
8. 구입한 로또와 당첨 번호를 비교한다.
    1. 일반번호들 일치하는 개수를 센다.
    2. 보너스번호 일치여부 확인한다.
9. 당첨통계 / 수익률 출력한다.
