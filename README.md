...
## Step4 피드백 반영
- [X] 원금에서 수동으로 생성한 로또의 개수만큼 빼고 잔금을 자동으로 처리하는 로직을 도메인으로 옮기기
  - LottoShop 도메인 추가
    - 로또를 수동, 자동으로 섞어서 안전하게 구매할 수 있도록 관리하는 도메인
    - [X] Money를 인자로 받아서 객체를 생성할 수 있다.
    - [X] 수동으로 LottoTicket을 하나씩 더할 수 있다.
        - [X] Money로 살 수 있는 수보다 더 많이 살 수 없다.
    - [X] 수동으로 LottoTicket을 사고 남은 돈으로 자동 LottoTicket을 구매할 수 있다.
      - 구매 후 구매한 모든 LottoTicket을 LottoTickets 컬렉션으로 받을 수 있다.