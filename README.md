...
- `로또 티켓(LottoTicket)`
    ...
    - [X] 다른 로또 티켓과 비교해서 일치하는 번호의 개수를 구할 수 있다. 
    
- `로또 티켓들(LottoTickets)`
    - [X] 당첨 로또와 비교해서 `등수(Rank)` 목록을 구할 수 있다.
...
- `넥스트 스탭 로또 기계(NextStepLottoMachine)`
    - [X] 문자열을 판독해서 수동 로또 티켓을 만들 수 있다. (구분자: ', ')
    - [X] 구매 가능한 만큼 자동 로또 티켓들을 만들 수 있다.
    - [X] 당첨 티켓과 보너스번호를 추가로 받아 우승 로또를 만들 수 있다.
    - [X] 구매 가능한 만큼 수동과 자동으로 로또 티켓을 생성할 수 있다.
    - [X] 구매 금액이 수동 로또 금액보다 낮을 경우 예외처리 한다.
