...
4. 문자열 계산기는 **사칙연산의 계산 우선순위가 아닌 입력 값에 따라 계산 순서가 결정**된다. 
즉, 수학에서는 곱셈, 나눗셈이 덧셈, 뺄셈보다 먼저 계산해야 하지만 이를 무시한다.
5. 예를 들어, `2 + 3 + 4 / 2`와 같은 문자열을 입력할 경우 `2 + 3 * 4 / 2` 실행 결과인 10을 출력해야 한다.
...
* 테스트할 수 있는 단위로 나누어 구현 목록을 만든다.
  ...
  * 곱셈
  * 나눗셈
  * 입력 값이 null이거나 빈 공백 문자인 경우 IllegalArgumentException throw
