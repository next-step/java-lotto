...
- 23.11.12
    - [ ] 로또 최소 금액이 1000원이기 때문에 1000원 미만의 금액을 입력했을 경우, 진행하지 못 하도록 막아보기
    - [ ] static으로만 구성된 클래스는 외부에서 인스턴스 생성을 막을 수 있게 생성자에 private 적용
    - [ ] 패키지 재분류 [util -> service]
    - [ ] Collections.unmodifiableSet 를 이용하여, Lotto 클래스의 번호들을 수정할 수 없도록 하기
    - [ ] lottoRank에서 count 값 비교하는 부분을 메서드 추출하기.
    - [ ] 로또 번호를 하나의 객체로 생각하고 추출하기.
