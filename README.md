...
- 코드 리뷰 반영
  - [X] `WinningLottoNumber` 매칭 로직 부여
  - [X] `Lotto` 생성을 `LottoFactory` 의 역할로 부여
- 구현 진행 사항
  - [X] `InputView`
    - [X] 수동 로또 구매장수 입력
    - [X] 수동 로또 번호 입력
    - [X] 입력값 자체 검증 로직
  - [X] `AutoLottos`
    - [X] ~~수동 로또 장수 제외한 자동 로또 생성 로직~~
    - [X] 생성할 자동 로또 장수를 받아서 처리로 변경
  - [X] `Lottos`
    - [X] 자동 로또 및 수동 로또 모두 저장
  - [X] `ManualLottos`
    - [X] 수동 로또를 생성한다.
    - [X] 수동 구매한 로또수와 수동 로또 입력값이 맞지 않다.
    - [X] 지불한 금액보다 수동으로 구입하려는 로또가 더 많다.
  - [X] `ManualLottosValidator`
    - [X] 수동 로또 검증 로직
      - [X] 수동 로또 구매 장수가 입력값의 수보다 크다.
      - [X] 수동 로또 구매 장수가 입력값의 수보다 작다.