...
[ ] 유저로부터, 수동으로 구매할 로또 수를 입력 받을 수 있다.
- [ ] Invalid한 로또 구매 수를 입력 받았을경우, 에러메시지를 표시하고 유저에게 재입력을 요청 할 수 있다.
	- [ ] 로또 구매 수가 , Null인 경우
	- [ ] 로또 구매 수가 , Empty인 경우
	- [ ] 로또 구입 수가, Non-Numeric한 경우
	- [ ] 입력 받은 로또 구매 수만큼 , 로또 구입이 불가능한 경우(로또 구매수 x 로또 1장 가격 > 로또 구입 금액)

[ ] 유저가 입력한 수동 로또 구매수만큼, 로또 번호를 입력 받을 수 있다.
- [ ] Invalid한 로또 번호를 입력 받았을경우, 에러메시지를 표시하고 유저에게 재입력을 요청 할 수 있다.
	...
	- [ ] 입력 받은 로또 수가, 로또 구매수와 일치하지 않을 경우

[ ] 입력 받은 금액, 수동 로또 구매번호를 토대로, 구입 가능한 만큼 로또를 생성 할 수 있다. (수동 + 자동)
- [ ] 수동 로또 구매수만큼 수동 로또 생성
- [ ] 입력받은 금액 - (수동 로또 구매수 x 로또 1장 가격) 만큼, 자동 로또 생성
...
- [x] 유저로부터, 로또 구입 금액을 입력 받을 수 있다.
- [ ] 유저로부터, 수동으로 구매할 로또 수를 입력 받을 수 있다.
- [ ] 유저로부터, 수동으로 구매할 로또 번호 목록을 입력 받을 수 있다.
- [x] 유저로부터, 지난 주 당첨 번호를 쉼표 구분으로 입력 받을 수 있다.
- [x] 유저로부터, 보너스 볼을 입력 받을 수 있다.

[x] ResultView
- [ ] 수동 + 자동로또 번호로 구매한 총 로또 구매수를 출력 할 수 있다.
