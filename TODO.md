...
2. [ ] 입력한 문자열 값에 따라 사칙연산을 수행한다
3. [ ] 입력 문자열의 숫자와 사칙 연산 사이에는 반드시 빈 공백 문자열이 있다고 가정한다
4. [ ] 나눗셈의 경우 결과 값을 정수로 떨어지는 값으로 한정한다
5. [ ] 문자열 계산기는 사칙연산의 계산 우선순위가 아닌 입력 값에 따라 계산 순서가 결정된다
