...
---
## 코드리뷰
- 현재 InputView 에서 도메인 객체들과 결합되어 있는 상태 (Input을 날것으로 받고 Controller 에서 변환하는 방식으로 변경하기)
- Lotto → Manual / Auto 분리한 것에 대한 리뷰
  - 현재 설계 내용으로 보면 Manual 과 Auto 의 차이는 생성자의 차이만 존재
  - 큰 차이는 없는데 벌써부터 상속으로 구조를 잡아버리면 이후 설계 변경이나 확장 시 제약이 생길 수 있음
  - 상속 대신 `정적 팩토리 메서드` 고려해보기
  - 만약 리뷰 내용으로 구조 변경한다면 결과 출력 시 Manual 과 Auto 의 개수를 출력하는 부분에 대해서도 고려해야 함.
